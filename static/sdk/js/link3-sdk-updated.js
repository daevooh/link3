...
class Link3SDK {
    ...
    async getBalance() {
        ...
        try {
            ...
            const response = await this._apiCall('/balance/');
    ...
    async recordInteraction(actionType, metadata = {}) {
        ...
        try {
            // Updated to use real API endpoint
            const response = await this._apiCall('/interaction/', {
    ...
    async getWallets() {
        ...
        try {
            // Updated to use real API endpoint
            const response = await this._apiCall('/wallets/');
    ...
    async verifyWallet(walletId, signature) {
        ...
        try {
            // For demo purposes, we'll continue using this endpoint
    ...
    async getRedemptions(options = {}) {
        ...
        try {
            // Updated to use real API endpoint
            const response = await this._apiCall('/redeem-blockchain/', options);
    ...
    async createRedemption(redemptionData) {
        ...
        try {
            // Get the wallet address for this wallet ID
            const walletsResponse = await this.getWallets();
            const wallet = (walletsResponse || []).find(w => w.id === redemptionData.walletId);
            ...
            // Updated to use real API endpoint
            const response = await this._apiCall('/redeem-blockchain/', {
                amount: redemptionData.amount,
                wallet_id: redemptionData.walletId,
    ...
    async getRedemptionDetails(redemptionId) {
        ...
        try {
            // Using modified endpoint for demo compatibility
            const response = await this._apiCall(`/redeem-blockchain/${redemptionId}`);
    ...
    async getTokenInfo() {
        try {
            // Using modified endpoint for demo compatibility
            const response = await this._apiCall('/tokens/');
